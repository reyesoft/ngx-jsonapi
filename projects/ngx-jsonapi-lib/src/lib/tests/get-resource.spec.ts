...
describe("core methods", () => {
    ...
    it("getResourceService should return the instantiated service from resourceServices related to the type passed as arument", () => {
        let test_service: TestService = new TestService();
        let test_service_instance: Service = core.getResourceServiceOrFail(
            "test_resources"
        );
    ...
    it(`service's get method should get the requested resource from the back end if it's not cached or the TTL has ended`, async () => {
        ...
        test_response_subject.next(
            new HttpResponse({ body: test_resource.toObject() })
        );
    ...
    it(`resource should have the correct hasOne and hasMany relationships corresponding to the back end response's included resources,
        ...
        test_resource_has_many_relationship_1.relationships.test_resources.data.push(
            { id: "4", type: "test_resources" }
        );
        ...
        await test_service
            ...
            .then(resource => {
                ...
                expect(resource.attributes.name).toBe("test_name");
                expect(
                    resource.relationships.test_resource instanceof
                        DocumentResource
                ).toBeTruthy();
                expect(
                    resource.relationships.test_resources instanceof
                        DocumentCollection
                ).toBeTruthy();
                expect(
                    (<DocumentResource>resource.relationships.test_resource)
                        .data.id
                ).toBe("2");
                expect(
                    (<DocumentResource>resource.relationships.test_resource)
                ...
                expect(
                    (<DocumentCollection>resource.relationships
                        .test_resources).data.find(
                ...
                let has_one_relationship:
                    | Resource
                    | null
                    | undefined = (<DocumentResource>resource.relationships
                    .test_resource).data;
                let has_many_relationship: Array<
                    Resource
                > = (<DocumentCollection>resource.relationships.test_resources)
                    .data;
                expect(
                    (<TestResource>has_one_relationship.relationships
                        .test_resource.data).id
                ).toBe("4");
                expect(
                    (<TestResource>has_many_relationship[0].relationships
                        .test_resources.data[0]).id
                ).toBe("4");
            });
    });
    ...
    it(`if the back end sends a hasOne relationship with a null data property, it should be set as null in the resulting resource`, async () => {
        ...
        let test_service: TestService = new TestService();
        test_response_subject.next(
            new HttpResponse({ body: { data: test_resource } })
        );

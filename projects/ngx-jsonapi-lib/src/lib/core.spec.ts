...
import {
    StoreService as JsonapiStore,
    StoreService
} from "./sources/store.service";
...
describe("core methods", () => {
    ...
    it("duplicateResource method should duplicate a resource and add the requested relationships (if present in the original reource)", () => {
        ...
        has_one_relationship_resource.attributes.data =
        ...
        has_many_relationship_resource.attributes.data =
        ...
        has_many_relationship_resource_2.attributes.data =
            "this is a has MANY relationship";

        original_resource.addRelationship(
            has_one_relationship_resource,
            "has_one"
        );
        original_resource.addRelationships(
            [has_many_relationship_resource, has_many_relationship_resource_2],
            "has_many"
        );

        let resource_copy: CustomResource = core.duplicateResource(
            original_resource
        );
        expect(resource_copy.id.includes("new_")).toBeTruthy();
        expect(resource_copy.attributes.data).toBe("this is a resource");
        expect(
            (<DocumentResource>resource_copy.relationships.has_one).data.id
        ).toBe("2");
        expect(
            (<DocumentResource>resource_copy.relationships.has_one).data
                .attributes.data
        ).toBe("this is a has ONE relationship");
        expect(resource_copy.relationships.has_many.data[0].id).toBe("3");
        expect(
            resource_copy.relationships.has_many.data[0].attributes.data
        ).toBe("this is a has MANY relationship");
        expect(resource_copy.relationships.has_many.data[1].id).toBe("4");
        expect(
            resource_copy.relationships.has_many.data[1].attributes.data
        ).toBe("this is a has MANY relationship");

        let resource_copy_with_duplicated_relationships: CustomResource = core.duplicateResource(
            original_resource,
            "has_one",
            "has_many"
        );
        expect(
            resource_copy_with_duplicated_relationships.id.includes("new_")
        ).toBeTruthy();
        expect(
            resource_copy_with_duplicated_relationships.attributes.data
        ).toBe("this is a resource");
        expect(
            (<DocumentResource>resource_copy_with_duplicated_relationships
                .relationships.has_one).data.id.includes("new_")
        ).toBeTruthy();
        expect(
            (<DocumentResource>resource_copy_with_duplicated_relationships
                .relationships.has_one).data.attributes.data
        ).toBe("this is a has ONE relationship");
        expect(
            resource_copy_with_duplicated_relationships.relationships.has_many.data[0].id.includes(
                "new_"
            )
        ).toBeTruthy();
        expect(
            resource_copy_with_duplicated_relationships.relationships.has_many
                .data[0].attributes.data
        ).toBe("this is a has MANY relationship");
        expect(
            resource_copy_with_duplicated_relationships.relationships.has_many.data[1].id.includes(
                "new_"
            )
        ).toBeTruthy();
        expect(
            resource_copy_with_duplicated_relationships.relationships.has_many
                .data[1].attributes.data
        ).toBe("this is a has MANY relationship");
    });
});

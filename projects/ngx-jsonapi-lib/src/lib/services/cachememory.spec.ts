...
describe("Cache Memory deprecation and live conditions", () => {
    it("clearCache", () => {
        ...
        let collection_on_memory: DocumentCollection = cachememory.getOrCreateCollection(
            "authors"
        );
        expect(collection_on_memory.cache_last_update).toBe(
    ...
    it("getResource() should return the stored resource", () => {
        ...
        let author_on_memory: Author = <Author>cachememory.getResource(
            "authors",
            author.id
        );
    ...
    it("getResource() should return null when the requested resource does not exist", () => {
        let cachememory: CacheMemory = CacheMemory.getInstance();
        let author_on_memory: Resource | null = cachememory.getResource(
            "authors",
            "some non stored author"
        );
    ...
    it("getResourceOrFail() should return the stored resource if exists", () => {
        ...
        let author_on_memory: Author = <Author>cachememory.getResourceOrFail(
            "authors",
            author.id
        );
        expect(author_on_memory).toBeTruthy();
    ...
    it("getResourceOrFail() should throw an error if the requested resource does not exist", () => {
        let cachememory: CacheMemory = CacheMemory.getInstance();
        expect(() => {
            cachememory.getResourceOrFail(
                "authors",
                "new_" + Math.floor(Math.random() * 6)
            );
        }).toThrow(
            new Error("The requested resource does not exist in cache memory")
        );
    });
    ...
    it("getOrCreateResource() should return the requested resource", () => {
        ...
        let author_on_memory: Resource = cachememory.getOrCreateResource(
            "authors",
            author.id
        );
    ...
    it("resource cache_last_update with update_lastupdate", () => {
        ...
        expect(author_on_memory.cache_last_update).toBe(
            author.cache_last_update
        );
    });

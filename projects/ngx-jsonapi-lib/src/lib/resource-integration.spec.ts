...
describe("Resource delete", () => {
    ...
    it("should send a DELETE request", async () => {
        let httpClientDeleteSpy: jasmine.Spy = jest
        ...
        await book
            ...
            .toPromise()
            .then(data => {
                expect(httpClientDeleteSpy.calls.mostRecent().args[0]).toBe(
                    "DELETE"
                );
            });
    });
...
describe("Resource save", () => {
    ...
    it("include_get should be included in the URL, but not in the request data", async () => {
        ...
        expect(http_request_spy.calls.mostRecent().args[1]).toBe(
            "http://yourdomain/api/v1/books/book_1?include=author"
        );
        expect(
            http_request_spy.calls.mostRecent().args[2].body.include
        ).toBeFalsy();
    });
    ...
    it("include_get should be included in the request data, but not in the URL", async () => {
        ...
        expect(http_request_spy.calls.mostRecent().args[1]).toBe(
            "http://yourdomain/api/v1/books/book_1"
        );
        expect(
            http_request_spy.calls.mostRecent().args[2].body.included
        ).toBeTruthy();
        expect(
            http_request_spy.calls.mostRecent().args[2].body.included.length
        ).toBe(1);
        expect(
            http_request_spy.calls.mostRecent().args[2].body.included[0].id
        ).toBe("author_1");
    });

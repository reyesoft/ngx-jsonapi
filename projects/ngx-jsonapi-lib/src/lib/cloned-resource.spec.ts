...
describe("ClonedResource save", () => {
    ...
    it(
        "should save only dirty attributes",
        waitForAsync(() => {
            let http_client_spy: jasmine.Spy = jest
            ...
            let author_clone: ClonedResource<Author> = new ClonedResource(
                author
            );
            test_response_subject.next(
                new HttpResponse({ body: author_clone.toObject() })
            );
            author_clone.attributes.name = "Luis";
            author_clone.save().subscribe(author_data => {
                expect(
                    http_client_spy.calls.mostRecent().args[2].body
                ).toMatchObject({
                    data: {
    ...
    it(
        "should save only dirty HAS ONE relationships",
        waitForAsync(() => {
            let http_client_spy: jasmine.Spy = jest
                .spyOn(HttpClient.prototype, "request")
                .and.callThrough();
            ...
            test_response_subject.next(
                new HttpResponse({ body: book_clone.toObject() })
            );
    ...
    it(
        "should save only dirty HAS MANY relationships",
        waitForAsync(() => {
            let http_client_spy: jasmine.Spy = jest
                .spyOn(HttpClient.prototype, "request")
                .and.callThrough();
...
describe("CloneResource properties changes", () => {
    ...
    it("Changing clone HAS ONE relationships", () => {
        ...
        expect(book.relationships.author.data.attributes.name).not.toBe(
            book_clone.relationships.author.data.attributes.name
        );
    });
    ...
    it("Changing clone HAS MANY relationships", () => {
        ...
        expect(author.relationships.books.data.length).not.toBe(
            author_clone.relationships.books.data.length
        );
    });
});

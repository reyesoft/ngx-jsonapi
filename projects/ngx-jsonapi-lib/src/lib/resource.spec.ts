...
describe("resource", () => {
    ...
    it("should save the resource without relationships that dont refer to a resource or mean to remove the relationship", async () => {
        ...
        let exec_spy = jest
            .spyOn(Core, "exec")
            .mockReturnValue(of({ data: response }));
        await resource.save();
        resource.relationships = {};
        ...
        expect(exec_spy).toHaveBeenCalledWith(
            "1234",
            "PATCH",
            expected_resource_in_save,
            true
        );
    ...
    it("toObject method should parse the resouce in a new IDocumentResource", () => {
        ...
        jest
            .spyOn(Resource.prototype, "getService")
            .mockReturnValue(mocked_service_data);
        ...
        let to_object_resource: IDocumentResource = new_resource.toObject(
            params
        );
        expect(to_object_resource.data.id).toBe("1");
        expect(to_object_resource.data.type).toBe("main");
        expect(
            to_object_resource.data.attributes &&
                to_object_resource.data.attributes.main_attribute
        ).toBe("123456789");
        ...
        expect(included && included[0] && included[0].type).toBe(
...
describe("resource.toObject() method", () => {
    it("(toObject) If the service has a parseToServer method, ir should be applied in toObject method", () => {
        ...
        jest
            .spyOn(Resource.prototype, "getService")
            .mockReturnValue(mocked_service_data as Service<Resource>);
        let new_resource: Resource = new Resource();
        new_resource.type = "main";
        new_resource.id = "1";
        new_resource.attributes = { main_attribute: "123456789" };
    ...
    it("(toObject) If a relationship is not in the include param, it should not be included in the resulting include field", () => {
        jest
            .spyOn(Resource.prototype, "getService")
            .mockReturnValue({} as Service<Resource>);
        let new_resource: Resource = new Resource();
        new_resource.type = "main";
        new_resource.id = "1";
        new_resource.attributes = { main_attribute: "123456789" };
    ...
    it("(toObject) hasMany empty and builded relationship should return an emtpy relationship", () => {
        ...
        const relationshipsAfterRemove =
            bookObjectAfterRemove.data.relationships;
        const photosAfterRemove =
            relationshipsAfterRemove && relationshipsAfterRemove.photos;
        const photosDataAfterRemove =
            photosAfterRemove && photosAfterRemove.data;
...
describe("resource.save() method", () => {
    it('if set, te save method should send the "meta" property when saving a resource', async () => {
        ...
        jest
            .spyOn(resource, "getService")
            .mockReturnValue(serviceMock as Service<Resource>);
        jest.spyOn(PathBuilder.prototype, "applyParams");
        resource.id = "1234";
        resource.type = "tests";
        resource.attributes = { name: "test_name" };

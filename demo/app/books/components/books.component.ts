...
export class BooksComponent {
    ...
    public constructor(protected authorsService: AuthorsService, protected booksService: BooksService) {
        booksService
            .all({
                page: { number: 2 },
                include: ['author' /* , 'photos' */] // @todo: when photos is included, store dont work
            })
            .subscribe(books => {
                this.books = books;
        ...
        books$.subscribe(
            books => {
                ...
                let books2 = this.booksService.all(success2 => {
                    console.log('BooksRequest#2 received (author data from cache)', <Resource>(
                        books2[Object.keys(this.books)[1]].relationships.author.data
                    ));
                });
